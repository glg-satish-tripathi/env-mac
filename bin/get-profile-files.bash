...
DATA="$( \
	...
	| jq -rM --from-file <(cat <<- DOC
		map(
			...
			| (
					.fields | from_entries
					 | {type: .type, workdir: .workdir, file_perm: .file_perm, dir_perm: .dir_perm}
				)
				+ {id: .id, name: .name}
...
for ITEM in ${DATA}; do
	...
	NAME="$(_item '.name')"
	ITEM_ID="$(_item '.id')"
	WORKDIR="$(_item '.workdir')"
	FILE_PERM="$(_item '.file_perm')"
	DIR_PERM="$(_item '.dir_perm')"
	...
	for ATTACHMENT in ${ATTACHMENTS}; do
		...
		WORKDIR="$(sed -E 's|[/]{2,}|\/|g' <<< "${HOME}/${WORKDIR}")"
		NEW_FILE="$(sed -E 's|[/]{2,}|\/|g' <<< "${WORKDIR}/${FILE}")"
		mkdir -p "${WORKDIR}"
		chmod "${DIR_PERM}" "${WORKDIR}"
		rm -f "${NEW_FILE}"
		bw get attachment "${ATTACHMENT_ID}" \
			--raw \
			--itemid "${ITEM_ID}" \
			> "${NEW_FILE}"
		chmod "${FILE_PERM}" "${NEW_FILE}"
